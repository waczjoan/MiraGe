...
class FlatGaussianModelSlices(GaussianModel):
    ...
    def get_xyz(self):
        return torch.stack(
            [self._xyz[:, 0], self._xyz[:, -2], self._xyz[:, -1]],
            axis=1
        )
    ...
    def create_from_pcd(self, pcd: BasicPointCloud, spatial_lr_scale: float):
        ...
        fused_point_cloud = torch.tensor(np.asarray(pcd.points)[:, [0, 1, 2]]).float().cuda()
